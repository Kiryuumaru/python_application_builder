...
class Dependency(metaclass=Meta):
    def __init__(self):
    ...
    def get_services(self, service_type: type[TDependency]) -> List[TDependency]:
        return self.application_builder.get_services(service_type)

    def get_service(self, service_type: type[TDependency]) -> TDependency:
        return self.application_builder.get_service(service_type)

    def get_factories(self, factory_type: type[TDependency]) -> List[DependencyFactory[TDependency]]:
        return self.application_builder.get_factories(factory_type)

    def get_factory(self, factory_type: type[TDependency]) -> DependencyFactory[TDependency]:
        return self.application_builder.get_factory(factory_type)
...
class DependencyFactory(Generic[TDependency]):
...
class ApplicationBuilder:
    def __init__(self, log_dir: str = os.path.join(os.getcwd(), "logs")):
        ...
        self.services: Dict[str, List[TDependency]] = {}
        self.factories: Dict[str, List[DependencyFactory[TDependency]]] = {}
    ...
    def _wire_dependency(self, dependency: TDependency, name: Union[str, None] = None):
    ...
    def _wire_dependency_factory(self, dependency_factory: DependencyFactory[TDependency], name: Union[str, None] = None):
    ...
    def add_service(self, service_type: type[TDependency], name: Union[str, None] = None):
        service = service_type()
        self._wire_dependency(service, name)
    ...
    def get_factories(self, factory_type: type[TDependency]) -> List[DependencyFactory[TDependency]]:
        ...
        factories: List[DependencyFactory[TDependency]] = []
        for factory in self.factories[key]:

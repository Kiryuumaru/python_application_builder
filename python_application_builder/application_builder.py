...
class DependencyCore:
    ...
    def get_configuration(self, key: str, default: Any = None) -> Any:
        ...
        """
        if key in self._application_host.configurations:
            return self._application_host.configurations[key]
    ...
    def get_services(self, service_type: type[TDependency]) -> Dict[str, TDependency]:
        """
        Retrieve all registered services of the specified type.
    ...
    def get_factories(self, factory_type: type[TDependency]) -> Dict[str, DependencyFactory[TDependency]]:
        """
        Retrieve all registered factories for the specified type.
...
class ApplicationHost:
    """
    Holds the application's configuration, registered services, factories, and workers.
    """
...
class ApplicationBuilder:
    """
    Main entry point for configuring and running the application.
    Provides methods to register services, factories, workers, and configurations.
    """
    def __init__(self, log_dir: str = os.path.join(os.getcwd(), "logs")):
        self._application_host = ApplicationHost(log_dir)
    ...
    def _wire_dependency(self, dependency: TDependency, key: Union[str, None] = None, name: Union[str, None] = None):
        ...
        dependency._application_host = self._application_host
        dependency.logger = loguru.logger.bind(context=dependency._dependency_name)
        dependency.logger.level("TRACE", color="<fg #444444>")
        dependency.logger.level("DEBUG", color="<fg #666666>")
        dependency.logger.level("INFO", color="<fg #FFFFFF>")
        dependency.logger.level("SUCCESS", color="<fg #00CC99>")
        dependency.logger.level("WARNING", color="<fg #FFBB00>")
        dependency.logger.level("ERROR", color="<fg #FF4444>")
        dependency.logger.level("CRITICAL", color="<fg #FF00FF>")
    ...
    def _wire_dependency_factory(self, dependency_factory: DependencyFactory[TDependency], key: Union[str, None] = None, name: Union[str, None] = None):
        ...
        dependency_factory._application_host = self._application_host
    ...
    def add_configuration(self, key: str, value: Any):
        ...
        """
        self._application_host.configurations[key] = value
    ...
    def add_service(self, service_type: type[TDependency], local_key: Union[str, None] = None, name: Union[str, None] = None):
        ...
        self._application_host.services_to_initialize.append(service)
        for key in service._dependency_keys:
            if key not in self._application_host.services:
                self._application_host.services[key] = {}
            if service._dependency_local_key in self._application_host.services[key]:
    ...
    def add_factory(self, factory_type: type[TDependency], local_key: Union[str, None] = None, 
        ...
        factory = DependencyFactory[factory_type](core_default_factory)
        self._wire_dependency_factory(factory, local_key, name)
        for key in factory._dependency_keys:
            if key not in self._application_host.factories:
                self._application_host.factories[key] = {}
            if factory._dependency_local_key in self._application_host.factories[key]:
                raise Exception(f"Factory '{factory._dependency_local_key}' already exists in '{key}'")
            self._application_host.factories[key][factory._dependency_local_key] = factory
    ...
    def add_worker(self, worker_type: type[TWorker], name: Union[str, None] = None):
        ...
        if key in self._application_host.workers:
            raise Exception(f"Worker '{key}' already exists")
        self._application_host.workers[key] = worker
    ...
    def run(self):
        ...
        loguru.logger.add(sys.stdout, colorize=True, format=self._application_host.log_format, level="TRACE")
        loguru.logger.add(os.path.join(self._application_host.log_dir, "main_{time:YYYY-MM-DD}.log"), 
                          format=self._application_host.log_format, level="TRACE",
                          rotation="1 day", retention="30 days")

        for key, value in os.environ.items():
            if key not in self._application_host.configurations:
                self._application_host.configurations[key] = value
                
        for service in self._application_host.services_to_initialize:
            self._initialize_service(service)

        threads = []
        for worker in self._application_host.workers.values():
